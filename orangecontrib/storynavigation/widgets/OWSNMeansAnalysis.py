...
class OWSNMeansAnalysis(OWWidget, ConcurrentWidgetMixin):
    name = 'Means Analysis'
    description = "Provides tools to help identify the main means of a story."
    ...
    def reset_story_elements(self, story_elements=None):
        if story_elements is not None:
            ...
            self.__action_analyze_means_wrapper()
    ...
    def __action_analyze_means_wrapper(self):
        self.start(self.action_analyze_means)


    def action_analyze_means(self, state: TaskState):
...
if __name__ == "__main__":
    ...
    previewer = WidgetPreview(OWSNMeansAnalysis)
