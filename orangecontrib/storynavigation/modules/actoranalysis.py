...
class ActorTagger:
    ...
    def __calculate_custom_tag_frequencies():
    ...
    def calculate_metrics_customfreq_table(self, df):
        ...
        res = dict(
            sorted(
                self.__calculate_custom_tag_frequencies().items(), key=itemgetter(1), reverse=True
            )
        )
    ...
    def __prepare_story_elements_frame_for_filtering(self, story_elements_df):
        ...
        for col in strcols:
            ...
            if col == 'token_text':
                story_elements_df['token_text_lowercase'] = story_elements_df[col].str.lower()
                story_elements_df['token_text_lowercase'] = story_elements_df['token_text_lowercase'].astype(str)
    ...
    def __remove_custom_tag_columns(self, df):
        dropcols = []
    ...
    def generate_actor_analysis_results(self, story_elements_df):
        ...
        for word in story_elements_df[story_elements_df['story_navigator_tag'].isin(['SP', 'SNP'])]['token_text_lowercase'].unique().tolist(): # loop through unique words that are subjects of sentences in a story
            df_word = story_elements_df[story_elements_df['token_text_lowercase'] == str(word)]
            raw_freq_df = df_word.groupby(['storyid', 'segment_id', 'token_text_lowercase'])['token_text_lowercase'].agg('count').to_frame("raw_freq").reset_index()
            subj_freq_df = df_word.groupby(['storyid', 'segment_id', 'token_text_lowercase'])['is_sentence_subject_boolean'].agg('sum').to_frame("subj_freq").reset_index()
            agency_df = df_word.groupby(['storyid', 'segment_id', 'token_text_lowercase'])['active_voice_subject_boolean'].agg('sum').to_frame("agency").reset_index()        
            mean_subj_freq_df = df_word.groupby(['storyid'])['is_sentence_subject_boolean'].agg('mean').to_frame("mean_subj_freq").reset_index()
            combined_df = pd.merge(raw_freq_df, pd.merge(subj_freq_df, agency_df, on=['storyid', 'segment_id', 'token_text_lowercase'], how='outer'), on=['storyid', 'segment_id', 'token_text_lowercase'], how='outer')
            ...
            # and custom freq? --> another function / widget output
        
        result_df = self.__remove_custom_tag_columns(result_df)
        return result_df
