...
class OWSNSentimentAnalyzer(OWDataProjectionWidget):
    ...
    class Inputs(OWDataProjectionWidget.Inputs):
        sentiment_data = Input('Sentiment Data', Table)
    ...
    def initialise_sentiment_data_network(self, data: Table):
        self.sentiment_data = data
        df1 = table_to_frame(self.sentiment_data, include_metas=True)
        nodes = []
        nodes.extend(df1['source_entity'].tolist())
        nodes.extend(df1['target_entity'].tolist())
        ...
        for index, row in df1.iterrows():
            if row['polarity'] == 0:
                G.add_edge(node_id_dict[row['source_entity']], node_id_dict[row['target_entity']], color='green', weight=round(row['polarity'], 2))
            elif row['polarity'] < 0:
                G.add_edge(node_id_dict[row['source_entity']], node_id_dict[row['target_entity']], color='red', weight=round(row['polarity'], 2))
            else:
                G.add_edge(node_id_dict[row['source_entity']], node_id_dict[row['target_entity']], color='blue', weight=round(row['polarity'], 2))

        edge_labels = {(u, v): data['weight'] for u, v, data in G.edges(data=True)}

        # Create a list of edge colors based on the condition
...
if __name__ == "__main__":  # pragma: no cover
    table = Table("iris")
    WidgetPreview(OWSNSentimentAnalyzer).run(set_data=table,
